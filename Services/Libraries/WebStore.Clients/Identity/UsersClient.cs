...
namespace WebStore.Clients.Identity
{
    ...
    {
        public UsersClient(IConfiguration configuration) : base(configuration, WebApiAddresses.Identity.Users) { }
        ...
        {
            var creationSuccess = await (await PostAsync($"{ServiceAddress}/User", user, cancel))
               .Content
        ...
        public async Task SetPasswordHashAsync(User user, string hash, CancellationToken cancel)
        {
            ...
            await PostAsync(
                $"{ServiceAddress}/SetPasswordHash", new PasswordHashDTO { User = user, Hash = hash },
                cancel);
        }
